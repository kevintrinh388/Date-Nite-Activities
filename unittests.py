...
class FunctionTests(unittest.TestCase):
    ...
    def test_maps_search_zip(self):
        ...
        self.assertEqual(
            result,
            f"https://www.google.com/maps/embed/v1/search?key={MAPS_KEY}&q=30260",
        )
    ...
    def test_business_search_filter_rating4_no_price(self):
        """filter_rating = 4, api response contains some objects with NO PRICE"""
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "businesses": [
                {
        ...
        with patch("yelp.requests.get") as mock_get:
            mock_get.return_value = mock_response
            result = business_search("location", "term", 4, "1,2,3,4", "sort_by")
            self.assertEqual(result, expected_output)
